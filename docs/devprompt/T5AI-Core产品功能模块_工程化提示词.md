...
<todos>
TODO-1：确认 breathing-orbit-package.zip 与 witch-sleep-orb.zip 的解压产物结构（HTML 入口、JS 接口、素材清单）。
TODO-2：确认 5 个冥想 MP3 的时长、是否允许变速、版权/音色是否可云端 TTS 替代。
TODO-3：确认塔罗牌库范围（仅大阿尔卡那 22 张 or 含小阿尔卡那 78 张）以及正逆位是否都支持。
TODO-4：确认外部存储方案（SPI Flash / SD 卡 / 纯 H5 缓存），以决定白噪声与冥想音频能否本地缓存。
TODO-5：确认 LED 扩展硬件（是否通过排针外接 RGB LED 灯带或仅使用板载 P9 LED）。
</todos>
