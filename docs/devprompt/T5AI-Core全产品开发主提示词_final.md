...
> **使用方式**：从 `<role>` 开始整体复制粘贴。本文件为 *_final 终版：全部 {{占位符}} 已按定稿值替换（GPIO/延时/存储/LED/牌库），原文件保留在同目录。
...
- exit：S3（手机入座）、S0（超时 30s / 按钮-后；MVP 定稿 30s，Kconfig INTRO_TIMEOUT_S 可调）
...
- exit：S4（激活完成，固定延时 3s，Kconfig S3_TO_S4_DELAY_MS）、S0（按钮-下弹起=离座，立即）
...
| 事件 ID | 源 | 映射 | 优先级 | 去抖 |
|---|---|---|---|---|
| EVT_BTN_BACK | 按钮-后 | GPIO_3（J1 pin16，排针扩展） | 最高（强制回待机） | 50 ms |
| EVT_ALARM_TIMER | 系统 RTC | 闹钟时刻到达 | 最高（全局抢占） | — |
| EVT_BTN_DOWN | 按钮-下 | GPIO_4（J1 pin21，排针扩展，电平触发在位检测） | 高 | 100 ms |
...
- 固件：硬件 RNG 随机抽卡（牌库全 78 张（22 大阿尔卡那 + 56 小阿尔卡那），正逆位；卡牌索引 firmware-config/tarot_deck_index.json）；向 LLM 发 {card_name, orientation, spread} 求中文解读 → TTS 播报；按时间轴向 H5 发事件：tarot_shuffle → tarot_reveal{card_id, orientation} → tarot_interpret_start{text} → tarot_interpret_end；揭示期紫光呼吸→金色常亮；连续抽卡缓存最近 3 次避免重复。
...
<led_effect_spec>
基于 GPIO P9 PWM / soft-PWM；若外接 RGB（MVP 不外接，仅板载 P9；量产预留 I2C 灯带 GPIO_0/1）经 I2C/SPI 驱动芯片复用同一接口。
| 场景 | 效果 |
|---|---|
| 呼吸模块 | 随吸-屏-呼亮度变化 |
...
- TODO-D3 S1 引导视频退出/超时条件：**已决策=超时 30s / 按钮-后退出 / 手机入座直接跳 S3**；「魔罗抽卡」=「塔罗抽卡」（上下文确认，无独立功能）。
...
- TODO-D6 塔罗牌库范围：**已定稿=全 78 张 + 正逆位**（线上 webp 79 文件与 PocketTarotCards-Rounded png 80 文件双套已对齐，索引见 firmware-config/tarot_deck_index.json）。
- TODO-D7 素材包结构：**已消化，详见 docs/assets-manifest.md**（breathing/白噪声 HTML 入口与 JS 接口、五场景音轨清单、冥想 5 课 MP3 时长 110~126s、遗留缺口=塔罗音效.mp4 未附 + witch-sleep-orb 252MB 原包素材本体待人工导出）。
</todos>
